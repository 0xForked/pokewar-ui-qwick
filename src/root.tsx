...
export default component$(() => {
  ...
  const theme = useStore({ isDark: false });
  useClientEffect$(() => {
    const savedTheme = localStorage.getItem('theme') || "false"
    theme.isDark = Boolean((/true/i).test(savedTheme))
  })
  ...
  return (
    <QwikCityProvider>
      ...
      <body class={ theme.isDark ? "dark" : ""} lang="en">
        <button
          class="p-2 rounded focus:outline-none absolute right-0 top-0 px-5 py-5"
          aria-label="Toggle theme"
          onClick$={() => {
            theme.isDark = !theme.isDark
            localStorage.setItem("theme", String(theme.isDark))
          }}
        >
          {theme.isDark ? <svg
            class="w-5 h-5 fill-current text-yellow-300"
